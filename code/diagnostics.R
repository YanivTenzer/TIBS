...
B_vec <- seq(200, 1000, 200) # try different numbers of permutations
...
for(n in n_vec)
{
  ...
  for(i in c(1:iters))
  {
    ...
    {
      prms$B <- B_vec[b]
      pvals[i,b] <- TIBS_rcpp(biased.data$data, w.fun, test.type, prms)$Pvalue #  TIBS(biased.data, w.fun, cur.test.type, prms)
    }
  }
  ...
  jpeg(paste0("../figs/var_p_hat_", test.type, "_n_", n, ".jpg"), width = 400, height = 400)
  plot(B_vec, theoretical.std, type="l", col="green", lwd=5, xlab="B", ylab="st.d.", ylim=range(empirical.std, theoretical.std),
       main=paste0(test.type, " std(p.hat) for n=", prms$n, " p.val=", round(true.pval, 3) ))
  lines(B_vec, empirical.std, col="red")
  legend(max(B_vec)*0.7, max(empirical.std), c("theoretical", "empirical"), lwd=c(5,2), col=c("green", "red"))
  dev.off()
}
