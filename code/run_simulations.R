    ...
    w.fun <- c('truncation', 'truncation', 'truncation', 'truncation', 
               ...
               'exponent_minus_sum_abs', 'truncation', 'gaussian') # not good that we have only one simulation with positive W. Should add X+Y?
    ...
    for(s in run.dep) # Run all on the farm  
    {
      ...
      {
        prms = list(B=100, sample.size=n, iterations=100, plot.flag=0, alpha=0.05, sequential.stopping=0, 
    ...
    jpeg(paste0("../figs/check_valid_n_", n, "_dep_", dependence.type[s], "_w_",  w.fun[s],  "_perturb_grid_", prms$perturb.grid, ".jpg"), width = 400, height = 400)
